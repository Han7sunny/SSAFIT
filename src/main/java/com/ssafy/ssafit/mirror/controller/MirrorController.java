...
=======
    ...
    public ResponseEntity<?> getSchedule(@PathVariable String id) {
        try {
            List<MirrorRoutineRespDto> mirrorRoutineRespDtoList = mirrorService.getSchedule(id);
    ...
    public ResponseEntity<?> startBasicRoutine(@RequestBody RecordRegisterReqDto recordRegisterReqDto) {
        try {
    ...
    @ApiOperation(value = "특정 운동 하나만 실행하는 기능",
            ...
            response = CommonResp.class)
    public ResponseEntity<?> startOutOfRoutine(@RequestBody MirrorRecordGenerateReqDto mirrorRecordGenerateReqDto) {
        try {
