...
import { Modal, Button, Text, IconButton, Provider, Portal } from 'react-native-paper'
...
export default function GroupListScreen({navigation}) {
  ...
  const Filters = ['인원', '패널티','모집인원', '운동 기간']

  useEffect( async () =>{
    const data = (await axios.get('http://70.12.246.116:8080/group/recruit')).data;
    setLists(data);
    console.log(data);
  }, []);
 ...
 return (
  <Provider style={{flex: 1}}>
    <View style={{alignItems:'center', flexDirection: 'row', justifyContent:'space-between'}}>
      <Text variant="headlineLarge" style={{fontWeight:'bold', marginTop:10}}> 그룹 모집 </Text>
    ...
    <View style={{flexDirection: 'row', minHeight:50, maxHeight:50, backgroundColor:'red', padding: 0}}>
      {SelectFilter.map((item,idx) =>
        <Button 
          mode='contained'
          style={[styles.button, {width: Math.max((item.label.length+1) * 29,100)}]}
